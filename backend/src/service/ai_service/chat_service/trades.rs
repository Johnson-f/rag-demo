...
impl TradeChatService {
    ...
    /// 2. Query Qdrant for top N most similar trade chunks (OR fetch recent trades if temporal query)
    ...
    pub async fn chat(&self, request: TradeChatRequest) -> Result<TradeChatResponse> {
        ...
        let relevant_trades = if self.is_symbol_query(&request.message) {
        ...
        } else if self.is_temporal_query(&request.message) {
            // Fetch most recent trades from database (chronological)
            self.get_recent_trades_from_db(request.context_limit).await?
        } else {
            // Use semantic search via vector service
            self.vector_service
                .search_similar(&request.message, request.context_limit)
                .await
                .context("Failed to search for relevant trades")?
